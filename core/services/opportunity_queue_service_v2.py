...
def _build_reason(
    ...
    *,
    ...
    if phone_profile == "owner_like":
        ...
        bits.append("teléfono broker_like")
...
def _score_breakdown_text(
    *,
    ...
    return (
        ...
        f"precio {price_signal:.1f} + zona {zone_signal:.1f} + geo {geo_signal:.1f}"
    )
...
def get_opportunity_queue_v2(
    ...
    for event in events:
        ...
        score = event_base + recency + price_signal + zone_signal + geo_signal
        ...
        row = {
            ...
            "phone_profile": phone_profile,
            "contact_group_key": _contact_group_key(listing.contact_id if listing else None, contact_phone),
            "contact_group_label": _contact_group_label(listing.contact_id if listing else None, contact_phone),
...
def build_opportunity_groups(
    ...
    *,
    ...
    for group_key, group_rows in grouped.items():
        ...
        latest_dt = max(
            (ensure_utc_naive(row.get("event_datetime")) for row in group_rows if row.get("event_datetime")),
            default=None,
        )
        top_score = max(float(row.get("score") or 0.0) for row in group_rows)
        avg_score = round(sum(float(row.get("score") or 0.0) for row in group_rows) / len(group_rows), 2)
    ...
    *,
    ...
    for row in rows:
        if group_by == "zone" and (row.get("zone_label") or "Sin zona") == group_key:
            filtered.append(row)
        elif group_by == "contact" and (row.get("contact_group_key") or "sin_contacto") == group_key:
            filtered.append(row)
